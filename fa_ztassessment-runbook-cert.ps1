...
.NOTES
    Required modules in the Automation Account (runtime 7.2):
        ...
        - Az.Accounts
        - Az.KeyVault      (required for Key Vault certificate mode)
        - Az.Storage
        - ZeroTrustAssessment
...
param(
    ...
    # NOTE: no ValidateSet here - Azure Automation passes array parameters as
    # raw text (e.g. [Graph, Azure]), which is normalised below instead.
...
# ---------------------------------------------------------------------------
# 0. Normalise the Services parameter
#    Azure Automation passes arrays as raw command-line text, so this may
#    arrive as @('Graph','Azure'), '[Graph, Azure]', 'Graph,Azure', or split
#    into fragments like '[Graph'. Flatten and clean whatever turns up.
# ---------------------------------------------------------------------------
$ValidServices = @('All','Graph','Azure','AipService','ExchangeOnline','SecurityCompliance','SharePointOnline')

$Services = @($Services) -join ',' -replace '[\[\]"'']', '' -split ',' |
            ForEach-Object { $_.Trim() } |
            Where-Object { $_ }

# Match case-insensitively against the supported list
$Services = $Services | ForEach-Object {
    $s = $_
    $found = $ValidServices | Where-Object { $_ -ieq $s }
    if ($found) { $found }
    else { throw "Unknown service '$s'. Valid values: $($ValidServices -join ', ')" }
}

if (-not $Services) { $Services = @('Graph','Azure') }
Write-Output "Services requested: $($Services -join ', ')"
...
# ---------------------------------------------------------------------------
# 7. Disconnect every service session.
#    Without this the job never reaches "Completed": Connect-ZtAssessment
#    leaves sessions open (Exchange Online keeps a remoting session alive) and
#    the runbook process will not exit. Hybrid Worker jobs have no time limit,
#    so a hung job runs indefinitely and keeps the VM switched on.
# ---------------------------------------------------------------------------
Write-Output "Closing service sessions..."
...
Write-Output "Sessions closed."

# ---------------------------------------------------------------------------
# 8. Clean up: remove local report files (the worker disk persists between
#    jobs and the export contains sensitive tenant data) and the certificate
# ---------------------------------------------------------------------------
try {
