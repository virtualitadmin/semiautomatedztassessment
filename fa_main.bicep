...
@description('Publicly reachable URI of ztassessment-orchestrator.ps1 (raw file). Starts the VM, runs the assessment, deallocates the VM.')
param orchestratorContentUri string
...
@description('Services passed to scheduled runs. Use ["All"] for every pillar, or list them explicitly. SharePointOnline and AipService need extra roles/permissions - see the guide.')
param services array = ['Graph', 'Azure', 'ExchangeOnline', 'SecurityCompliance', 'SharePointOnline', 'AipService']
...
var orchestratorRunbookName = 'ZeroTrustAssessment-Orchestrator'
...
// The certificate is read by whichever identity the runbook authenticates as.
// On a Hybrid Runbook Worker, Connect-AzAccount -Identity resolves to the
// AUTOMATION ACCOUNT's managed identity (Automation injects it into the job
// context), not the VM's - so that is the identity that needs the role.
resource kvSecretsUserAutomation 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(keyVault.id, automation.id, '4633458b-17de-408a-b874-0445c86b69e6')
  scope: keyVault
  properties: {
...
// Also granted to the VM's own identity, so the runbook still works if it is
// ever run outside Automation (e.g. tested manually on the worker).
...
// ---------------------------------------------------------------------------
// NOTE: no Az modules are imported here on purpose.
// A PowerShell 7.2 runtime environment already ships the "Az" package
// (Az.Accounts, Az.Compute, Az.Automation and the rest). Importing individual
// Az.* modules as custom packages loads a second, different version of
// Az.Accounts alongside it and breaks every runbook in the account with:
//   "Unable to find type [...AzAssemblyLoadContextInitializer]"
// If a module genuinely is missing, add it through the portal so Azure
// resolves a compatible dependency chain.
// ---------------------------------------------------------------------------

resource orchestratorRunbook 'Microsoft.Automation/automationAccounts/runbooks@2023-11-01' = {
  parent: automation
  name: orchestratorRunbookName
  location: location
  tags: baseTags
  properties: {
    runbookType: 'PowerShell72'
    logProgress: false
    logVerbose: false
    description: 'Starts the worker VM, runs the assessment, then deallocates the VM'
    publishContentLink: { uri: orchestratorContentUri }
  }
}

resource varVmRg 'Microsoft.Automation/automationAccounts/variables@2023-11-01' = {
  parent: automation
  name: 'ZT-VmResourceGroup'
...
resource varVmName 'Microsoft.Automation/automationAccounts/variables@2023-11-01' = {
  parent: automation
  name: 'ZT-VmName'
  properties: { value: '"${vmName}"', isEncrypted: false }
}

resource varWorkerGroup 'Microsoft.Automation/automationAccounts/variables@2023-11-01' = {
  parent: automation
  name: 'ZT-WorkerGroup'
  properties: { value: '"${workerGroupName}"', isEncrypted: false }
}

resource varAaName 'Microsoft.Automation/automationAccounts/variables@2023-11-01' = {
  parent: automation
  name: 'ZT-AutomationAccountName'
  properties: { value: '"${automation.name}"', isEncrypted: false }
}

resource varAaRg 'Microsoft.Automation/automationAccounts/variables@2023-11-01' = {
  parent: automation
  name: 'ZT-AutomationResourceGroup'
  properties: { value: '"${resourceGroup().name}"', isEncrypted: false }
}

// Managed identity of the Automation account: start/stop the VM
resource aaVmContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(resourceGroup().id, automation.id, '9980e02c-c2be-4d73-94e8-173b1dc7cf3c')
  scope: resourceGroup()
  properties: {
    principalId: automation.identity.principalId
    principalType: 'ServicePrincipal'
...
// Managed identity of the Automation account: start runbooks / read jobs
resource aaAutomationContributor 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid(automation.id, automation.id, 'f353d9bd-d4a6-484e-a77a-8050b599b867')
  scope: automation
  properties: {
    principalId: automation.identity.principalId
    principalType: 'ServicePrincipal'
...
resource linkSchedule 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  ...
  properties: {
    ...
    arguments: '-Rg ${resourceGroup().name} -Aa ${automation.name} -Runbook ${orchestratorRunbookName} -Schedule ZT-Monthly -ContainerName ${containerName} -ServicesB64 ${base64(string(services))}'
    scriptContent: '''
      param($Rg, $Aa, $Runbook, $Schedule, $ContainerName, $ServicesB64)
      ...
      if ($existing) {
        ...
        # The ORCHESTRATOR runs in the Azure sandbox (no -RunOn): it starts the
        # VM, dispatches the assessment to the hybrid worker, then deallocates.
        Register-AzAutomationScheduledRunbook -ResourceGroupName $Rg -AutomationAccountName $Aa `
            -RunbookName $Runbook -ScheduleName $Schedule `
            -Parameters @{ ContainerName = $ContainerName; Services = $Services } | Out-Null
        Write-Output "Schedule '$Schedule' linked to orchestrator runbook '$Runbook'."
      }
    '''
  }
  dependsOn: [aaContributor, orchestratorRunbook, schedule, worker]
}
